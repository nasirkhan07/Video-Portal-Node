...
userModel.authUser = function(username, password){
	...
	User.findOne({username: username, password: md5(password)},function(err, dbuser) {
		if (err){
			results.reject(err);
		} 
...
//seeding database with default users
userModel.addNew = function(uname,pwd){
	var newUser = new User({username:uname, password:md5(pwd), activeSession:''});
	newUser.save(function(err, user) {
	  if(err) console.dir('error occured in populating database');
	});

	console.log('new user '+ uname+' created');
}
